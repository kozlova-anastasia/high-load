...
| HTTPS CPS | 6 676 |
| SSL TPS | 56 175 | 
| Throughput | 8.8 Гбит/с | 

Метрики из предыдущих разделов по доменам:

| Домен | Пиковый RPS | Пиковый CPS (30% от RPS) | Трафик через NGINX, Гбит/с | Расчет |
| :--- | :--- | :--- | :--- | :--- |
...
| EdgeЦентр  | Отдача статики, cdn | Аккредитованный Минцифры РФ провайдер облачных и edge-решений, который объединяет CDN, облачные сервисы (IaaS/PaaS), защиту от DDoS-атак и стриминг на базе собственной глобальной инфраструктуры |
...
| L7-балансировщики/ cdn | N + 1 резервирование, 4 ноды | При падении 1 ноды, оставшиеся покарывают всю нагрузку |
